...
class hexWave(object):
    ...
    def __init__(self, nbins=50, cmap='viridis', frames=100, interval=25, savefig=True):
        ...
        # Save to output dir
        if savefig:
            figpath = os.path.join('output', 'hexbin',f'hexbin_{self.colormap}_{self.array_size}.gif')
    ...
    def init(self):
        ...
        hist = self.ax.hexbin(self.x_init, self.y_init, bins=self.bins, gridsize=self.bins, cmap=self.colormap)
        return hist
    ...
    def animate(self, frame):
        ...
        hist = self.ax.hexbin(x,y, bins=self.bins, gridsize=self.bins,  cmap=self.colormap)
        return hist

if __name__ == "__main__":
    gt = hexWave(nbins=25, cmap='tab10', interval=200)
