...
class imageRecreate():
    ...
    def read_image(self, image_path: str, as_array: bool = True, convert_to_greyscale: bool = False):
        ...
        parameters:
            ...
            as_array : bool, default True; if True, a np.ndarray is returned.
                                If False, a Pillow.Image object is returned.

        returns:
            image : PIL Image object or np.array (if as_array set to True) ;
                        raw image data from source image,
                        can be manipulated with other class methods
        '''
    ...
    def entropy(self, image_path: str, savefig: bool = False):
        ...
        returns:
            entr_image : np.array containing pixel data for the input image
        '''
        ...
        # Plotting 3 panels: raw image, image with basic colormap,
        # entropy image with same colormap applied
        if self.sub_figs:
            f, (ax1, ax2, ax3) = plt.subplots(nrows=1, ncols=3,
                                              figsize=(9, 6), sharey=True)
            ...
            if savefig:
                fig_path = os.path.join(".", 'plots',
                                        'entropy',
                                        f'entropy_{im_name}_{self.cmap}.pdf')
    ...
    def segment_image(self, image_path: str,  n, thresh, c, savefig: bool = False):
        '''
        Creates segmentation (impressionist-style)
            recreation of an image using scikit-image
        ...
        if savefig:
            filename = f'segmentation_{im_name}_{n}_{thresh}_{c}.pdf'
            f.savefig(os.path.join('plots', 'segmentation', filename))
...
if __name__ == "__main__":
    # Get command line args if passed by the user, otherwise use default vals
    try:
        input_path = argv[1]
        cmap = argv[0]
    except:
        input_path = os.path.join(os.path.join('img',
